...
class PieChart extends Component{
  ...
  //form={this.props.form} energyScenario={this.props.energyScenario} climateModel={this.props.climateModel} climateScenario={this.props.climateScenario} 
  //filterstr={this.props.filterstr}historicInputState={this.props.historicInputState} 
  ...
  async componentDidMount()
  {
    ...
    else if(localStorage.getItem('form')=="Projected")
    {
      if(this.props.historicInputState.toLowerCase().includes("all us"))
      {
        var url="https://ewed.org:31567/ewedService/getFutureData/defaultViewData/"+this.props.energyScenario+"/fuelType/"+startYear+"/"+startmonthinInt+"/"+endYear+"/"+endmonthinInt+"/fuelTypes/"+this.props.filterstr;
      }
      else if(this.props.historicInputState.toLowerCase().includes("state"))
      {
  ...
  render() {
    ...
    else
    {
      ...
      {
        console.log(item);
        if(item["emission"]&&Number(item["emission"])>0)
        {
          myset2.add(item.filterName);
          data2.datasets[0]["data"].push(item["emission"])
        }
        if(item["generation"]&&Number(item["generation"])>0)
        {
          myset.add(item.filterName);
          data1.datasets[0]["data"].push(item["generation"]);
        }
        if(item["waterConsumption"]&&Number(item["waterConsumption"]>0))
        {
          myset3.add(item.filterName);
          data3.datasets[0]["data"].push(item["waterConsumption"])
        }
        if(item["waterWithdrawal"]&&Number(item["waterWithdrawal"])>0)
        {
          myset4.add(item.filterName);
          data4.datasets[0]["data"].push(item["waterWithdrawal"])
        }
      }
      // if(item["filterName"]==="Solar"||item["filterName"]==="Wind"||
      // item["filterName"]==="Biomass"||item["filterName"]==="Coal"||
      // item["filterName"]==="Natural Gas"||item["filterName"]==="Geothermal"
      // ||item["filterName"]==="Nuclear"||item["filterName"]==="Other"
      // ||item["filterName"]==="Water")
      // {
      //   myset2.add(item.filterName);
      //   data2.datasets[0]["data"].push(item["emission"])
      // }
